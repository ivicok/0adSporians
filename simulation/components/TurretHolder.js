...
class TurretHolder
{
	Init()
	{
		...
		// mod: counts turreted entities for further use
		this.units = 0;
		this.rowers = 0;
	}
	...
	OccupyTurretPoint(entity, requestedTurretPoint)
	{
		...
		// mod: the default stance somehow did not work for the horse archer "turret"
		cmpPositionOccupant.SetYRotation(cmpPositionSelf.GetRotation().y);
		...
		// mod: small boats should be slower with more passengers
		// currently 75% - 60% - 50% - 43%
		// technically, units passengers might be given oars and the value can be inverted :)
		let cmpIdentity = Engine.QueryInterface(this.entity, IID_Identity);
		let cmpOccId = Engine.QueryInterface(entity, IID_Identity);
		this.units = this.units + 1;
		if (cmpOccId && cmpOccId.HasClass("Rower"))
			this.rowers = this.rowers + 1;
		if (cmpIdentity && cmpIdentity.HasClass("Monoxylon"))
		{
			let cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
			cmpUnitMotion.SetSpeedMultiplier((1/((this.units/3)+1)) + this.rowers*0.15);
		}
	...
	LeaveTurretPoint(entity, forced, requestedTurretPoint)
	{
		...
		// mod: this is actually obsolete, as crucial turrets are not targeted now
		// still, the rider sometimes remains stuck in Map Editor
		if (!turretPoint.ejectable && forced) {
		...
		// mod: small boats should be slower with passengers
		let cmpIdentity = Engine.QueryInterface(this.entity, IID_Identity);
		let cmpOccId = Engine.QueryInterface(entity, IID_Identity);
		this.units = this.units - 1;
		if (cmpOccId && cmpOccId.HasClass("Rower"))
