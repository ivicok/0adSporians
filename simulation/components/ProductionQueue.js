...
ProductionQueue.prototype.AddItem = function(templateName, type, count, metadata)
{
	...
	if (type == "unit")
	{
		...
		// mod: gender randomizer
		let gnd = templateName.includes("_mle");
		if (gnd)
		{
			var rnd = Math.random();
			if (rnd > 0.5)
				templateName = templateName.replace("_mle", "_fem");
		}

		// mod: shaman chooses a specific variant for each god
		let shaman = templateName.includes("shaman");
		if (shaman) {
			let cmpTech = QueryOwnerInterface(this.entity, IID_TechnologyManager);
			if (cmpTech.IsTechnologyResearched("req_peraunu"))
				templateName = templateName.replace("shaman", "shaman_per");
			if (cmpTech.IsTechnologyResearched("req_welinsu"))
				templateName = templateName.replace("shaman", "shaman_wel");
		}
