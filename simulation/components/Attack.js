...
{
	// mod: check for animation control for the dragon
...
{
	// mod: used to remove turrets from target lists
	if (target) {
		let cmpId = Engine.QueryInterface(target, IID_Identity);
		if (cmpId && cmpId.HasClass("NonTarget"))
			return false;
	}
...
{
	// mod: no ranged attacks while swimming
	let cmpAI = Engine.QueryInterface(this.entity, IID_UnitAI);
	if (cmpAI && cmpAI.swimming == true)
		return "Melee";
...
Attack.prototype.StartAttacking = function(target, type, callerIID)
{
	...
	if (cmpVisual)
	{	
		// mod: swimming attack visuals are handled by UnitAI component
		let cmpAI = Engine.QueryInterface(this.entity, IID_UnitAI);
		if (cmpAI && cmpAI.swimming)
			this.target = target;
		else
...
Attack.prototype.StopAttacking = function(reason)
{
	...
	if (cmpVisual)
	{
		// mod: should prevent standing on water while swimming
		if (cmpAI && cmpAI.swimming)
...
Attack.prototype.PerformAttack = function(type, target)
{
	...
	// mod: if attacker is an inherited turret, its chassis should be considered the attacker
	let cmpId = Engine.QueryInterface(this.entity, IID_Identity);
	if (cmpId.HasClass("Turret")) {
		let cmpTurret = Engine.QueryInterface(this.entity, IID_Turretable);
	...
	// mod: helps with sustaining attack animations
	this.isAttacking = true;
