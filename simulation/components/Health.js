...
Health.prototype.Schema =
	...
	"<optional>" +
		...
		"</element>" +
	"</optional>" +
	// mod: randomized variant of the above
	"<optional>" +
		"<element name='IsTrophy'>" +
			"<data type='boolean'/>" +
		"</element>" +
	"</optional>" +
...
Health.prototype.Init = function()
{
	...
	// mod: life drain rate
	this.drainRate = this.GetDrainRate();
	
	// mod: controls biome variants
...
// mod: life drain rate
Health.prototype.GetDrainRate = function()
{
	if (this.drainRate > 1)
...
// mod: portion of dealt damage, returned to attacker as HP
// 25% by default
Health.prototype.ApplyDrain = function()
{
	this.drainRate = 4;
};

Health.prototype.StopDrain = function()
{
	this.drainRate = 1;
};

// mod: actor variants for cold biomes
Health.prototype.WarmClothes = function()
{
	let cmpVisual = Engine.QueryInterface(this.entity, IID_Visual);
	var rnd = Math.random();
	var winterVar = "winter_1";
	if (rnd <= 0.66 && rnd > 0.33)
		winterVar = "winter_2";
	else if (rnd > 0.66)
		winterVar = "winter_3";
	
	cmpVisual.SetVariant("key2", winterVar);
...
Health.prototype.TakeDamage = function(amount, attacker, attackerOwner)
{
	...
	// mod: gives the attacker a portion of lost HP (used by vampiric aura)
	let attackerHealth = Engine.QueryInterface(attacker, IID_Health);
	let thisId = Engine.QueryInterface(this.entity, IID_Identity);
	if (attackerHealth && thisId) {
		if (attackerHealth.drainRate > 1 && attackerHealth.IsInjured() && thisId.HasClass("Organic"))
			attackerHealth.Increase(amount/attackerHealth.drainRate);
	}
	...
	if (!this.hitpoints) {
		...
		// mod: create a trophy for the attacker (skulls!)
		// update: do not create trophies in water
		let cmpAI = Engine.QueryInterface(this.entity, IID_UnitAI);
		if (cmpAI)
		{
			if (!cmpAI.swimming && this.template.IsTrophy) {
				var rnd = Math.random();
				if (rnd > 0.5)
					this.CreateTrophy(attacker);
			}
		}
	}
...
Health.prototype.HandleDeath = function()
{
	...
	// mod: skulls have their own spawner
	if (this.template.SpawnEntityOnDeath && !this.template.IsTrophy)
...
// mod: an entity spawned on death for the attacker
// technically could also be used for Dune 2-like deviators or zombies...
Health.prototype.CreateTrophy = function(attacker)
{
	let cmpPosition = Engine.QueryInterface(this.entity, IID_Position);
	if (!cmpPosition.IsInWorld())
		return INVALID_ENTITY;
	...
	let cmpOwnership = Engine.QueryInterface(attacker, IID_Ownership);
