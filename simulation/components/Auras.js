...
{
	return (this.GetType(name) == "range" || this.GetType(name) == "vampiric" || this.GetType(name) == "winter");
};
...
// mod: vampiric aura
Auras.prototype.IsVampiricAura = function(name)
{
	return this.GetType(name) == "vampiric";
};
...
Auras.prototype.ApplyAura = function(name, ents)
{
	...
	// mod: life drain aura based on grapejuice's
	// attacks of affected units increase their health by small part of dealt damage
	if(name == "vampiric")
	{
		...
		for (let i = 0; i < length; i++) 
		{
			...
			if (cmpHealth != null)
				cmpHealth.ApplyDrain();
		}
	}

	// mod: enables biome variants
	if(name == "winter")
	{
		let length = ents.length;
		for (let i = 0; i < length; i++) 
		{
			let entity = ents.pop();
			let cmpId = Engine.QueryInterface(entity, IID_Identity);
			if (cmpId && cmpId.HasClass("Biome"))
			{
				let cmpHealth = Engine.QueryInterface(entity, IID_Health);
				if (cmpHealth != null)
					cmpHealth.WarmClothes();
			}
		}
	}
