...
TechnologyManager.prototype.CanProduce = function(templateName)
{
	...
	// mod: check for required buildings added
	// if the last req. building is destroyed, the queued production of the unit requiring it will continue (as resources are already allocated), but new training orders will not be allowed
	if (template.Identity) {
		if (template.Identity.RequiredTechBuilding) {
			let tech = TechnologyTemplates.Get(template.Identity.RequiredTechBuilding);
			if (!this.CheckTechnologyRequirements(DeriveTechnologyRequirements(tech, Engine.QueryInterface(this.entity, IID_Player).GetCiv()))) {
				this.researchedTechs.delete(template.Identity.RequiredTechnology);
				this.unresearchedAutoResearchTechs.add(template.Identity.RequiredTechnology);
				return false;
			}
		} else if (template.Identity.RequiredTechnology)
