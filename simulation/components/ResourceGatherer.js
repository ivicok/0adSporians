...
ResourceGatherer.prototype.StopGathering = function(reason)
{
	...
	// mod: trophies should not be gathered in the same way as other stuff
	// otherwise the gatherer returns after getting each one skull
	let res = this.GetLastCarriedType();
	if (res !== undefined && res.generic == "skull") 
		return;
...
ResourceGatherer.prototype.PerformGather = function(data, lateness)
{
	...
	// mod: only skulls from enemies should be collected
	if (type !== undefined && type.generic == "skull")
	{
		let cmpOwner = Engine.QueryInterface(this.entity, IID_Ownership);
		if (!IsOwnedByPlayer(cmpOwner.GetOwner(), this.target)) {
			this.StopGathering("OutOfRange");
			return;
		}
	}
