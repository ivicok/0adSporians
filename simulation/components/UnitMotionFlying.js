...
UnitMotionFlying.prototype.OnUpdate = function(msg)
{
	...
	if (this.landing)
	{
		if (this.speed > 0 && this.onGround)
		{
			...
			// mod: a walking animation on the ground
			let cmpAI = Engine.QueryInterface(this.entity, IID_UnitAI);
			if (cmpAI)
				cmpAI.SelectAnimation("walk");
			...
			// mod: of course it should be able to turn
			// canTurn = false;
		...
		else if (this.speed == 0 && this.onGround)
		{
			...
			else
			{
				...
				// mod: an idle animation when standing still
				let cmpAI = Engine.QueryInterface(this.entity, IID_UnitAI);
				if (cmpAI)
				{
					let cmpAttack = Engine.QueryInterface(this.entity, IID_Attack);
						if (cmpAttack && cmpAttack.isAttacking == true)
							cmpAI.SelectAnimation("attack_melee");
						else
							cmpAI.SelectAnimation("idle");
				}
		...
		{
			// mod: a landing animation?
			let cmpAI = Engine.QueryInterface(this.entity, IID_UnitAI);
			if (cmpAI)
				cmpAI.SelectAnimation("fly");
	...
	{					
		...
		if (this.speed < this.template.TakeoffSpeed && this.onGround)
		{
			...
			if (cmpAI) {
				let cmpAttack = Engine.QueryInterface(this.entity, IID_Attack);
				let cmpMotion = Engine.QueryInterface(this.entity, IID_Health);
					if (cmpAttack && cmpAttack.isAttacking == true) {
						cmpAI.SelectAnimation("attack_melee");
						// here it would be great to use a working FaceTowardsTarget()
					}
					else {
						if (this.speed == 0)
							cmpAI.SelectAnimation("idle");
		...
		{
			// mod: a flying animation in the air
				let cmpAI = Engine.QueryInterface(this.entity, IID_UnitAI);
				if (cmpAI)
					cmpAI.SelectAnimation("fly");
	...
	// mod: wow, let us attack once and run forward
